...
export default function Dashboard({
  ...
  const etiquetas = Array.from(etiquetasMap.entries())
    ...
    function cleanNumber(n: any): number {
      ...
      const datasets = almacenes
        .filter(alm => sede === 'todas' || alm === sede)
        .map((alm: string, i: number) => {
          const nombreAlmacen = ventasPorSedeMes.find(v => v.codalm === alm)?.nomalm || alm;

          const data = etiquetas.map((etiqueta) => {
            const suma = ventasFiltradas
              .filter((v: any) => {
                try {
          ...
          <Card>
            <CardHeader className="flex items-center gap-4">
                ...
                <CardDescription className="space-y-1 text-sm">
                    <p>Utilidad: <strong>${ventasFiltradas.reduce((acc, v) => acc + Number(v.utilid || 0), 0).toLocaleString()}</strong></p>
                    <p>
                    Rentabilidad:&nbsp;
                    <strong>
                        {totalVentas > 0
                        ? ((ventasFiltradas.reduce((acc, v) => acc + Number(v.utilid || 0), 0) / totalVentas) * 100).toFixed(2)
                        : '0.00'}%
                    </strong>
                    </p>
                </CardDescription>
            ...
            <CardHeader className="flex flex-col sm:flex-row gap-6">
              ...
              <div className="flex flex-col sm:flex-row justify-between w-full gap-6">
                <div className="flex-1">
                  ...
                  <CardDescription className="space-y-1 text-sm">
                    {(() => {
                      const notas = ventasPorDocumento.filter(item =>
                        ['Dev-Ind A Caja', 'Nota Cre. Indep', 'N.Cre A Factura'].includes(item.tipdoc));
                ...
                <div className="flex-1 text-left sm:text-right">
                  ...
                  <CardDescription className="space-y-1 text-sm">
                    {(() => {
                      const facturas = ventasPorDocumento.filter(item =>
                        ['Factura de Caja', 'Factura de Venta', 'N.Deb A Factura'].includes(item.tipdoc));
